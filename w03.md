...
**Hash Table :** The hash table takes a value computed from the file name and returns a pointer to the file. It decreases the directory search time. The insertion and deletion process of files is easy. The major difficulty is hash tables are its generally fixed size and hash tables are dependent on hash function on that size. <br>
* * *

8. **[What Is a Journaling File System?](https://www.maketecheasier.com/journaling-in-file-system/)** <br>
